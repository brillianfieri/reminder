...
const EditReminder = ({reminder, reminderParams, fetchReminderData, localeDate}: {reminder:ReminderData, reminderParams:ReminderParams, fetchReminderData:FetchReminderData, localeDate:Date|null }) => {
   ...
   const [selectTime, setSelectTime] = useState(reminder.time_flag==1 ? true: false)
   const [reminderDate, setReminderDate] = useState<string>(localeDate ? localeDate.toISOString().split('T')[0] : new Date().toISOString().split('T')[0])
   const [reminderTime, setReminderTime] = useState<string>((reminder.time_flag && localeDate) ? ('0'+ localeDate.getUTCHours()).slice(-2)+":"+ ('0'+ localeDate.getUTCMinutes()).slice(-2) : "00:00")
  ...
  if((reminder.reminder_date && !reminderDate) && (reminder.time_flag == 1 && !reminderTime)){
    return <></>
  }
   return (
      <>
            ...
            <form onSubmit={handleSubmit} className="p-5 rounded-xl bg-white dark:bg-zinc-900" >
                ...
                {selectDate ? 
                    <div className='flex items-center justify-center mb-6'>
                        ...
                        <div>
                            <button type="button" onClick={()=>{setSelectDate(false); setSelectTime(false);}}  className="text-black dark:text-white">
                ...
                {selectTime ? 
                    <div className='flex items-center justify-center mb-6'>
                        ...
                        <div>
                            <button type="button" onClick={()=>{setSelectTime(false);}}  className="text-black dark:text-white">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="w-6 h-6">
                                    <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
                                </svg>
                            </button>
                ...
                <div className='flex mb-5'>
                    ...
                    {selectTime ? null: <div><button type="button" onClick={()=>{setSelectDate(true); setSelectTime(true);}}  className="text-black bg-slate-300 hover:bg-slate-400 focus:outline-none focus:ring-4 font-medium rounded-full text-sm px-5 py-2 text-center mr-2 mb-2 dark:bg-slate-700 dark:hover:bg-slate-800 dark:text-white">Add Time</button></div>}
                </div>
