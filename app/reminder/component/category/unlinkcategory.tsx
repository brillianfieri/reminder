...
const UnlinkCategory = ({reminderParams, setReminderParams}: {reminderParams:ReminderParams, setReminderParams:SetReminderParams}) => {
   ...
   const [isOpen, setIsOpen] = useState(false)
    ...
    const handleSubmit = async (event: React.ChangeEvent<HTMLFormElement>) => {
        ...
        const response = await fetch('/api/reminder/category/unlink', {
            method: 'POST',
            headers: {
        ...
        // Change log
        Log(`revoked their access from ${reminderParams.categoryName} category.`)
                ...
                <div className='flex flex-direction-row content-center justify-center'>
                    <div>
                        <h2 className='text-xl font-bold dark:text-white pb-5'>
                            Remove your access from {reminderParams.categoryName}?
                        </h2>
                    </div>
                </div>
                <div className='pt-5 flex flex-direction-row content-center justify-center'>
                    <button type="submit" className="text-white bg-red-700 hover:bg-red-800 focus:outline-none focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-2 mb-2">Remove</button>
                </div>
            </form>
