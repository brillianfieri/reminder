...
const DeleteCategory = ({reminderParams, setReminderParams}:{reminderParams:ReminderParams, setReminderParams:SetReminderParams}) => {
   const [isOpen, setIsOpen] = useState(false)
    ...
    const handleSubmit = async (event: React.ChangeEvent<HTMLFormElement>) => {
        ...
        const response = await fetch('/api/reminder/category/delete', {
            method: 'POST',
            headers: {
        ...
        // Change log
        Log(`deleted category ${reminderParams.categoryName}  and its contents.`)
         ...
         <form onSubmit={handleSubmit} className="p-5 rounded-xl bg-white dark:bg-gray-900" >
                <div className='flex flex-direction-row content-center justify-center'>
                    <div>
                        ...
                        Delete {reminderParams.categoryName}?
                    </h2>
                    </div>
                </div>
